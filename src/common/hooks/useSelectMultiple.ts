...
export function useSelectMultiple<T extends string | number>(
  ...
  function handleSelectItem(selectedId: T, isChecked: boolean) {
    if (isChecked) {
      ...
      const isCheckedAll = listIdInCurrentPage.every((id) =>
        newestIds.includes(id)
      );
  ...
  function handleCheckAll(isChecked: boolean) {
    ...
    if (isChecked) {
      const newUniqueId = getUniqueArray([
        ...selectedIds,
        ...listIdInCurrentPage,
      ]) as T[];
      setSelectedIds(newUniqueId);
    } else {
      const newUniqueId = selectedIds.filter(
        (id) => !listIdInCurrentPage.includes(id)
      ) as T[];
      setSelectedIds(newUniqueId);
    }
  }
